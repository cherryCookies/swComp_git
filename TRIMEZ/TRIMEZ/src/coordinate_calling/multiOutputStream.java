...
public class multiOutputStream{
	...
	static FileOutputStream fo_gz = null;
	static Path ref_add;
	static String UPLOAD_PATH = "output";
	static int MAX_SIZE = 1073741824; // 1GB
	static String fileName_gz = "output.fa.gz"; 
	static String fileName = "output.fa"; 
	
	public static extract multiOutput(String lastz_add,String ref_seq_add, String trg_seq_add) throws IOException {
		String commands;
		String resAddress = "output";
		...
		commands=lastz_add+" "+ref_seq_add+" "+trg_seq_add+" --step=100 --notransition --exact=100 --format=general:name1,start1,length1,name2,start2,strand2";
		...
		try{
			...
			*/
			...
			Process oProcess2=operator.exec(commands);
			InputStreamReader isr=new InputStreamReader(oProcess2.getInputStream());
			BufferedReader stdOut   = new BufferedReader(isr);
			BufferedReader stdError = new BufferedReader(isr);
			///System.out.println(stdOut.readLine());
	       File file = new File(resAddress+"/alignment.txt"); // File객체 생성
	       ...
	       */
	       ...
	       ///bw.close();
			while ((s = stdError.readLine()) != null) {
				...
				System.exit(oProcess2.exitValue());
			}
	...
	public static String streamUsingURL(String species, String chromosome, String url) {			
			...
			try {
				...
				*/
				...
				bi = new BufferedInputStream(new URL(DOWNLOAD_URL).openStream());
				fo_gz = new FileOutputStream(UPLOAD_PATH + "/" + fileName_gz); // 파일이 저장될 위치
				Path ref_add_gz = Paths.get(fo_gz.toString());
				///fo = new FileOutputStream(UPLOAD_PATH + "/" + fileName); // 파일이 저장될 위치
				///ref_add= Paths.get(fo.toString());
				
				
				byte buffer[] = new byte[1024*1024]; // 최대 1KB씩 저장
				int len;
				long fileSize = 0;
				
				while((len = bi.read(buffer, 0, buffer.length)) != -1) { // 파일을 읽으며 buffer에 최대 buffer길이만큼 저장
					fo_gz.write(buffer, 0, len); // buffer의 0~len에 담긴 파일 내용을 저장
				...
				*/
				...
				return UPLOAD_PATH + "/" + fileName_gz;
				
			} catch (Exception e) {
				e.printStackTrace();
			}
			///String fo = UPLOAD_PATH + "/" + fileName;
			///System.out.println(fo_gz.toString());
			///GZipExample.main(fo_gz.toString(), fo);
			return UPLOAD_PATH + "/" + fileName_gz;
	}
